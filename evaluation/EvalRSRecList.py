...
class myRecList(EvalRSRecList):
    def miss_rate_gini(self,
                                      y_preds: pd.DataFrame,
                                      y_test: pd.DataFrame,
                                      slice_info: pd.DataFrame,
                                      slice_key: str):
        mr_per_slice = self.miss_rate_at_k_slice(y_preds, y_test, slice_info, slice_key).to_dict()
        mr_per_slice_values=list(mr_per_slice.values())
        mad = np.abs(np.subtract.outer(mr_per_slice_values, mr_per_slice_values)).mean()
        # Relative mean absolute difference
        rmad = mad/np.mean(mr_per_slice_values)
        # Gini coefficient
        g = 0.5 * rmad
        return {'gini': g,  **mr_per_slice }
    
    @rec_test(test_type='track_popularity_gini')
    def track_popularity_gini(self):
        track_id = self._y_test['track_id']
        track_activity = self._x_train[self._x_train['track_id'].isin(track_id)]
        track_activity = track_activity.groupby('track_id', as_index=True, sort=False)[['user_track_count']].sum()
        track_activity = track_activity.loc[track_id]
        bins=track_activity.user_track_count.quantile(np.linspace(0,1,6)[1:-1]).values
        track_activity['bin_index'] = np.digitize(track_activity.values.reshape(-1), bins)
        print(np.unique(track_activity['bin_index'].values))
        track_activity['bins'] = bins[track_activity['bin_index'].values - 1]
        return self.miss_rate_gini(self._y_preds, self._y_test, track_activity, 'bins')